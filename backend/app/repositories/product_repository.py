...
class ProductRepository:
    ...
    def get_multiple_by_ids(self, pruduct_ids: List[int]) -> List[Product]:
        query = select(Product).where(Product.id.in_(pruduct_ids).options(joinedload(Product.category)))
        result = self.db.execute(query)
        return result.scalars().all()
